...
- **SSR Warning**: In SSR environments like Nuxt 3, module-level state is shared across all incoming requests on the server. This means one user's data can leak into another user's response — a critical security issue. Use Nuxt's `useState` instead of module-level refs for SSR-safe singleton state.
